...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.title("🔐 私有智库访问授权")
        st.text_input("授权码错误", type="password", on_change=password_entered, key="password")
        return False
    else:
        return True
...
if st.session_state.current_chat_id:
    ...
    st.info("请在下方输入问题。建议长文本分段提问，以获得更稳定的生成效果。")
        ...
        with st.spinner("正在检索并生成回答..."):
            try:
                ...
                if not combined_context.strip():
                    response = "知识库中未找到相关资料，以下基于通用模型回答：\n\n"
                ...
                prompt_tmpl = ChatPromptTemplate.from_template("你是一个智库助手。请根据以下资料回答。\n资料：{context}\n问题：{question}")
            ...
            except Exception as e: st.error(f"连接或执行出错：{e}")

# --- 7. 导出 Word 功能 ---
if st.session_state.current_chat_id and st.session_state.all_chats[st.session_state.current_chat_id]["messages"]:
    st.divider()
    doc = Document()
    doc.add_heading('对话记录导出', 0)
    for msg in st.session_state.all_chats[st.session_state.current_chat_id]["messages"]:
        role = "用户" if msg["role"] == "user" else "AI 助手"
        doc.add_paragraph(f"【{role}】:", style='Caption')
        doc.add_paragraph(msg["content"])
    
    bio = BytesIO()
    doc.save(bio)
    st.download_button(label="📥 点击下载当前对话 (Word)", data=bio.getvalue(), file_name="ai_export.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
